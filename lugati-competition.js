...
function renderStudent(el){
 ...
 const available=ARENA_ORDER.filter(k=>rounds[k]);
 const body=tab==='history'?studentHistory():tab==='leaderboard'?seasonBoard():tab==='indicators'?indicatorAccount():
 '<section class="bg-white/95 rounded-[2rem] p-5 sm:p-6 border border-white/20"><div class="flex items-center justify-between gap-3 mb-4"><div><span class="text-sm text-emerald-700 font-black">🎮 ألعابك الآن</span><h2 class="text-xl font-black text-slate-900 mt-1">الألعاب المرسلة لك</h2><p class="text-sm text-slate-500 mt-1">نعرض الألعاب المتاحة فعلًا فقط؛ كل مهارة تظهر كمحطة قبل المواجهة النهائية.</p></div><div class="hidden sm:block text-4xl">🎮</div></div>'+(available.length?'<div class="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">'+available.map(k=>studentArenaCard(k,rounds[k])).join('')+'</div>':'<div class="rounded-3xl bg-slate-50 border p-8 text-center"><div class="text-4xl">🎯</div><div class="font-black text-slate-800 mt-3">لا توجد لعبة مرسلة الآن</div><div class="text-xs text-slate-400 mt-1">ستظهر اللعبة هنا مباشرة عندما يفتحها المعلم.</div></div>')+'</section>';
...
function studentArenaCard(k,r){
 const m=arenaMeta(k),th=arenaTheme(k),style='--accent:'+th.accent+';';
 if(!r)return'';
 const a=r.attempt,st=r.effective_status,finishedOpen=st==='open'&&a&&a.status!=='in_progress';
 const badgeText=finishedOpen?'أنهيتها ✓':statusText(st);
 const badgeClass=finishedOpen?'tk-badge tk-badge-success':statusClass(st);
 let action='';
 ...
 else if(finishedOpen)action='<div class="mt-4 p-3 bg-emerald-50 text-emerald-800 border border-emerald-100 rounded-2xl text-sm font-black text-center">✓ أنهيت الجولة — تظهر النتيجة عند إغلاق المعلم</div>';
 ...
 return '<div style="'+style+'" class="comp-arena-card '+(st==='open'&&!finishedOpen?'open ':'')+'rounded-3xl p-5 min-h-[235px] flex flex-col"><div class="flex items-start justify-between"><div class="arena-icon w-12 h-12 rounded-2xl flex items-center justify-center text-2xl">'+m.icon+'</div><span class="text-sm px-2.5 py-1 rounded-full font-black '+badgeClass+'">'+badgeText+'</span></div><div class="mt-3"><b class="block text-slate-900">'+esc(m.label)+'</b><span class="text-sm font-black" style="color:'+th.accent+'">'+th.mood+'</span></div><div class="flex flex-wrap gap-1.5 mt-3"><span class="tk-badge tk-badge-success">'+ar(r.game_config?.indicator_count||r.selected_indicators?.length||0)+' مهارات</span><span class="tk-badge tk-badge-info">'+ar(r.game_config?.stage_count||0)+' مراحل</span></div><h3 class="text-sm font-black text-slate-700 mt-3 leading-5">'+esc(r.title)+'</h3><p class="text-sm text-slate-400 mt-1">'+(finishedOpen?'أكملت ما عليك في هذه الجولة':(r.close_at?'تغلق '+fmtDate(r.close_at):'الإغلاق يحدده المعلم'))+'</p><div class="mt-auto">'+action+'</div></div>';
}
