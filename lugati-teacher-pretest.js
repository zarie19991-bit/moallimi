...
const S={token:'',scope:'all',subject:'reading',templates:[],cache:{reading:null,math:null,science:null},selected:new Set(),bundleTitle:'',sendingBundle:false,loading:false,error:''};
...
function renderTabs(){const x=document.getElementById('pretestTabs');if(!x)return;x.innerHTML=tabsHtml();x.querySelectorAll('[data-pretest-subject]').forEach(b=>b.onclick=()=>{S.subject=b.dataset.pretestSubject;S.templates=S.cache[S.subject]||[];S.selected.clear();S.bundleTitle='';renderTabs();load()})}
...
function toggleTemplate(id){
 const k=String(id);if(S.selected.has(k))S.selected.delete(k);else S.selected.add(k);render();
}
async function sendBundle(){
 const selected=selectedTemplates(),meta=SUBJECTS[S.subject];
 if(selected.length<2){alert('اختر مؤشرين على الأقل لإنشاء رحلة إتقان مدمجة.');return}
 const title=(S.bundleTitle||`رحلة إتقان ${meta.name} — ${selected.length} مؤشرات`).trim();
 const preview=selected.slice(0,8).map(t=>`المؤشر ${t.global_indicator||t.indicator_index}`).join('، ')+(selected.length>8?`، +${selected.length-8}`:'');
 if(!confirm(`إرسال رحلة واحدة مدمجة إلى جميع الطلاب؟\n\n${title}\n${preview}\n\nسيكمل الطالب المؤشرات بالترتيب داخل رحلة واحدة.`))return;
 S.sendingBundle=true;render();
 try{
   const d=await post('send_bundle',{template_ids:selected.map(t=>t.id),bundle_title:title});
   S.selected.clear();S.bundleTitle='';S.cache[S.subject]=null;
   alert(`تم إنشاء «${d.bundle_title}» وإرسالها إلى ${d.students} طالبًا.\nعدد المؤشرات: ${d.indicator_count}`);
   await load(true);
 }catch(e){alert(e.message)}
 finally{S.sendingBundle=false;render()}
}
function bundleBuilderHtml(){
 const rows=selectedTemplates(),meta=SUBJECTS[S.subject],count=rows.length;
 const defaultTitle=`رحلة إتقان ${meta.name} — ${count} مؤشرات`;
 return `<section class="bundle-builder">
   <div class="bundle-builder-top"><div><h3>🧩 إنشاء رحلة إتقان مدمجة</h3><p>اختر المؤشرات؛ ستصل للطالب كتدريب واحد مرتب، ويشرح كل مؤشر قبل الانتقال إلى أسئلته.</p></div><span class="bundle-count">${count} محدد</span></div>
   <div class="bundle-controls"><input id="bundleTitleInput" maxlength="140" placeholder="${esc(defaultTitle)}" value="${esc(S.bundleTitle)}"><button id="sendBundleBtn" class="bundle-send" ${count<2||S.sendingBundle?'disabled':''}>${S.sendingBundle?'جارٍ الإرسال…':'إرسال رحلة واحدة'}</button><button id="clearBundleBtn" class="bundle-clear" ${!count?'disabled':''}>مسح الاختيار</button></div>
   ${count?`<div class="bundle-preview">${rows.slice(0,12).map((t,i)=>`<span class="bundle-chip">${i+1}. مؤشر ${t.global_indicator||t.indicator_index}</span>`).join('')}${count>12?`<span class="bundle-chip">+${count-12} أخرى</span>`:''}</div>`:''}
 </section>`;
}
function wireBundleBuilder(box){
 const input=box.querySelector('#bundleTitleInput');if(input)input.oninput=()=>{S.bundleTitle=input.value};
 box.querySelector('#sendBundleBtn')?.addEventListener('click',sendBundle);
...
function render(){
 ...
 box.innerHTML=`
 <div class="pretest-note"><b>رحلة الإتقان:</b> تستطيع إرسال مؤشر واحد، أو تحديد عدة مؤشرات وإنشاء <b>رحلة واحدة مدمجة</b>. في الرحلة المدمجة تُرتَّب المؤشرات تلقائيًا حسب رقمها، ويشرح الطالب كل مؤشر ثم يتدرب عليه ويثبت إتقانه قبل الانتقال إلى التالي.</div>
 ${bundleBuilderHtml()}
 ...
 <div class="pretest-grid">${S.templates.map(t=>{const st=t.last_dispatch?.stats||{},g=t.global_indicator||t.indicator_index,selected=S.selected.has(String(t.id));return`<article class="pretest-item ${selected?'selected':''}">
   ...
   <div class="pretest-actions"><button data-select-template="${t.id}" class="pretest-select">${selected?'✓ مضاف إلى الرحلة المدمجة':'＋ أضف إلى رحلة مدمجة'}</button><button data-send-pretest="${t.id}" class="pretest-send">إرسال هذا المؤشر منفردًا</button></div>
 </article>`}).join('')}</div>`;
 wireBundleBuilder(box);
 box.querySelectorAll('[data-send-pretest]').forEach(b=>b.onclick=async()=>{
   ...
   if(!confirm(`إرسال رحلة ${meta.name} — المؤشر ${t.global_indicator} لجميع الطلاب؟\n\n${t.indicator_text}`))return;
   const old=b.textContent;b.disabled=true;b.textContent='جارٍ الإرسال…';
   try{const d=await post('send_all',{template_id:t.id});b.textContent=`تم الإرسال إلى ${d.students} طالبًا ✓`;S.cache[S.subject]=null;await load(true)}
