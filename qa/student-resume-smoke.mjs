...
let starts=0,resumes=0,finishes=0;
await page.route('**/functions/v1/nafes-exam',async route=>{
 ...
 if(body.action==='assessment_finish'){finishes++;return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({...state(),submitted:true})});}
...
if(!stored.browserSession)throw new Error('anonymous browser session missing');
await page.reload({waitUntil:'domcontentloaded',timeout:60000});
await page.waitForSelector('#player:not([hidden])',{timeout:30000});
if(starts!==1)throw new Error(`student registered again ${starts}`);
if(resumes!==1)throw new Error(`resume count ${resumes}`);
if(finishes!==0)throw new Error(`refresh submitted the active attempt ${finishes} time(s)`);
console.log('PASS student_resume_without_reregistration_or_submit',JSON.stringify({starts,resumes,finishes}));
await browser.close();
