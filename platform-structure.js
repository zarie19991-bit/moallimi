...
function ensureApprovalButton(list){
 ...
 if(btn)return btn;
 btn=document.createElement('button');btn.innerHTML='<i>✓</i><span>التوصيات والاعتماد</span>';btn.onclick=()=>{state.achievementOfficialPage='approval-plus';renderAchievement()};list.appendChild(btn);return btn;
}
function groupAchievementIndex(){
 const list=document.querySelector('#achievementArea .ach-index-list');if(!list)return;
 const all=[...list.querySelectorAll('button')];ensureApprovalButton(list);const buttons=[...list.querySelectorAll('button')];
 list.innerHTML='';
 GROUPS.forEach(g=>{
...
function signaturesHtml(block){
 const fields=block.content?.fields||[];return `<section class="ach-block"><h3>${esc(block.title||'اعتماد الملف')}</h3><div class="ach-sign-grid">${fields.map(f=>`<div><span>${esc(f.label||'')}</span><b>${esc(f.value||'________________')}</b></div>`).join('')}</div></section>`;
}
function renderApproval(){
 const paper=document.querySelector('#achievementArea .ach-paper');if(!paper)return;
...
const prev=window.renderAchievement;
if(typeof prev==='function')window.renderAchievement=function(){const r=prev.apply(this,arguments);enhanceAchievement();return r};
