...
function ensurePanel(force=false){
 ...
 const signature=tests.map(t=>`${t.id}:${t.status}:${t.access_code||''}`).join('|');
 let panel=area.querySelector('.nafes-bank-created-panel');
 if(panel&&!force&&panel.dataset.signature===signature)return;
 if(!panel){panel=document.createElement('section');panel.className='card nafes-bank-created-panel';const bottom=area.querySelector('.nafes-tests');if(bottom)bottom.before(panel);else area.appendChild(panel)}
...
if(typeof originalBuild==='function'){
 window.buildArabicReadyTest=async function(){
  ...
  creating=true;toast('جارٍ إنشاء الاختبار وإضافة أسئلته...');
  try{await originalBuild();setTimeout(()=>{switchSection('nafes');window.setNafesSubject?.('arabic');setTimeout(()=>ensurePanel(true),80)},120)}
  catch(e){toast('تعذر إنشاء الاختبار. أعد المحاولة مرة واحدة.',true)}
  finally{creating=false}
 };
}
document.addEventListener('click',e=>{if(e.target.closest('[data-section="nafes"],.nafes-subject'))setTimeout(()=>ensurePanel(false),80)});
setTimeout(()=>ensurePanel(false),500);
})();