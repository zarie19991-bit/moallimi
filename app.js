...
function renderContent(){const s=subject(current),matched=s.outcomes.filter(matchOutcome);let visibleIndicators=0;const groups=[];for(const d of domains(s)){const outcomes=matched.filter(o=>o.domain===d);if(!outcomes.length)continue;const html=outcomes.map(o=>{const inds=o.indicators.map((t,i)=>({t,i})).filter(x=>indicatorMatch(x.t,o));visibleIndicators+=inds.length;if(!inds.length)return'';return `<details class="outcome" ${query?'open':''}><summary><span class="code">${esc(o.code)}</span><div class="outcome-title"><b>${esc(o.title)}</b><small>${esc(o.subdomain)} · ${o.indicators.length} مؤشرات</small></div><span class="chev">⌄</span></summary><div class="indicator-list">${inds.map(x=>`<div class="indicator"><span class="num">${x.i+1}</span><p>${esc(x.t)}</p></div>`).join('')}</div></details>`}).join('');if(html)groups.push(`<section class="domain"><div class="domain-head"><h4>${esc(d)}</h4><span>${outcomes.length} ناتج تعلم</span></div>${html}</section>`)}
