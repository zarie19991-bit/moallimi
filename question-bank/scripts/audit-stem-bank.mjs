...
for(const file of files){
 ...
 for(const q of doc.questions){
  ...
  if(expectedLevel==='reasoning'&&(/اختار طالب/.test(q.question_text)||q.options.some(x=>/إجابة الطالب/.test(x))))errors.push(`${file} q${q.question_no}: meta reasoning wrapper`);
 ...
 if(modelStems.size!==15)errors.push(`${file}: distinct stems ${modelStems.size}/15`);
