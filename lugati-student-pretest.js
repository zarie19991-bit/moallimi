...
function injectHero(){
 ...
 if(prog){
   prog.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px">${Object.entries(SUBJECTS).map(([k,m])=>{const st=subjectStats(k),pct=st.total?Math.round(st.ready/st.total*100):0;return`<div class="bg-white border rounded-3xl p-5"><div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-size:20px">${m.icon}</div><div style="font-weight:900;margin-top:3px">${m.name}</div></div><b style="color:#047857">${st.ready}/${st.total}</b></div><div style="height:8px;background:#e2e8f0;border-radius:999px;overflow:hidden;margin-top:12px"><div style="height:100%;width:${pct}%;background:#10b981"></div></div><div style="font-size:10px;color:#64748b;margin-top:7px">${pct}% إتقان للمؤشرات الكلية</div>${st.assigned?`<button class="journey-secondary" data-progress-subject="${k}" style="margin-top:10px;width:100%">التفاصيل</button>`:''}</div>`}).join('')}</div>`;
...
function renderMap(){
 ...
 ${cur?`<div class="journey-current"><div><small style="color:#0284c7;font-weight:900">المؤشر النشط • رقم ${cur.global_indicator}</small><h3>${esc(cur.indicator_text)}</h3><p>${stateText(cur.state,cur.assignment?.current_stage)} • جميع المؤشرات التي أرسلها المعلم تبقى متاحة لك.</p></div><button class="journey-primary" data-open-assignment="${cur.assignment.id}">أكمل الآن</button></div>`:''}
...
function progress(step){const names=['١. مفتاح السر','٢. جرّب معي','٣. تحدي المؤشر','٤. اختبار الخروج','٥. الإتقان'];return `<div class="journey-progress">${names.map((n,i)=>`<div class="journey-step ${i<step?'done':i===step?'active':''}">${i<step?'✓ ':''}${n}</div>`).join('')}</div>`}
function renderJourney(){const b=document.getElementById('journeyBody');if(!b||!S.journey)return;if(S.stage==='secret'){const t=S.journey.template;b.innerHTML=`${progress(0)}<div class="journey-panel"><div class="secret-icon">💡</div><div class="activity-title">مفتاح السر • المؤشر ${t.global_indicator}</div><h2>${esc(t.student_title)}</h2><p class="sub">${esc(t.indicator_text)}</p><div class="gold-card"><b>🔑 قاعدة الحل الذهبية</b><br>${esc(t.golden_rule)}</div><div class="learn-grid"><div class="learn-box"><h3>👀 كيف أعرف أن السؤال على هذا المؤشر؟</h3><ul>${(t.recognition_cues||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div class="learn-box"><h3>🔄 كيف قد تتغير صيغة السؤال؟</h3><ul>${(t.question_patterns||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div class="learn-box" style="grid-column:1/-1"><h3>🧭 خطوات الحل مهما تغير النص</h3><ol>${(t.solution_steps||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ol></div></div><div class="transfer-card"><b>قاعدة الانتقال لسؤال جديد:</b><br>${esc(t.transfer_rule||t.golden_rule)}</div><div class="trap-card"><b>⚠️ فخ نافس الشائع</b><br>${esc(t.common_trap)}</div><button id="goGuided" class="journey-action">فهمت الطريقة — دربني على الأنواع الثلاثة</button></div>`;document.getElementById('goGuided').onclick=()=>{S.stage='guided';const g=S.journey.guided||[],r=S.journey.responses||{};let i=g.findIndex(q=>!(r[q.id]?.activity_correct&&r[q.id]?.objective_correct));S.guidedIndex=i<0?g.length:i;resetActivity();renderGuided()};return}renderGuided()}
...
function renderRetention(){
 ...
 if(S.retention.status==='passed'){box.innerHTML='<div class="feedback ok">ثبت الإتقان بعد المراجعة المؤجلة ✓</div>';return}
