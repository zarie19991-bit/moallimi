 ...
 if(primary){
   if(combined){primary.innerHTML=combinedJourneyCard(combined,false);primary.querySelector('[data-open-combined]')?.addEventListener('click',()=>openCombinedJourney(combined.id));}
     ...
     primary.innerHTML=`<div class="skill-focus">
       <div class="skill-focus-top"><div class="skill-focus-kicker">مهارتك اليوم • ${meta.icon} ${meta.name} • رقم ${cur.global_indicator}</div><div class="skill-focus-state">${stateText(cur.state,cur.assignment?.current_stage)}</div></div>
       <h2>${esc(g?.title||cur.indicator_text)}</h2>
       <div class="skill-official"><b>المؤشر الرسمي:</b> ${esc(cur.indicator_text)}</div>
       ...
       <div class="skill-focus-action"><div class="skill-focus-note">سترى الصياغة بطرق مختلفة داخل التدريب؛ المطلوب أن تتقن طريقة الحل لا أن تحفظ سؤالًا واحدًا.</div><button id="openCurrentJourney">ابدأ تدريب المهارة ←</button></div>
     </div>`;
...
function renderMap(){
 ...
 S.map=S.maps[S.activeSubject]||[];
 const meta=SUBJECTS[S.activeSubject],assigned=S.map.filter(x=>x.assignment),ready=S.map.filter(x=>x.state==='ready').length,teacherBundles=S.teacherBundles?.[S.activeSubject]||[],assessmentCombined=S.assessmentJourneys?.[S.activeSubject]||null;
