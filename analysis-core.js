...
(function (root, factory) {
  ...
  'use strict';
  /*
   * Student-level support follows the school/report rule:
   * every score below 70% is either "بحاجة إلى دعم" or "غير متقن".
   * Indicator mastery remains 80%.
   */
  const THRESHOLDS = Object.freeze({ mastered: 80, near: 70, support: 50 });
  const LEVELS = Object.freeze([
    { key: 'mastered', label: 'متقن', min: 80 },
    { key: 'near', label: 'قريب من الإتقان', min: 70 },
  ...
  const needsSupport = percent => { const p = num(percent); return p !== null && p < THRESHOLDS.near; };
  ...
  function studentRows(attempts, filters = {}) {
    const scoped = new Map(), allByIdentity = new Map();
    for (const a of attempts) {
      const identity = a.studentIdentity || studentIdentity(a);
      if (!allByIdentity.has(identity)) allByIdentity.set(identity, []);
      allByIdentity.get(identity).push(a);
      if (!matchesContext(a, filters)) continue;
      if (!scoped.has(identity)) scoped.set(identity, []);
      scoped.get(identity).push(a);
    }
    const rows = [];
    for (const [key, history] of scoped) {
      ...
      const priorPool = (allByIdentity.get(key) || []).filter(a => matchesContext(a, { ...filters, test: '' }) && latest && compareTime(a, latest) < 0).sort(compareTime).reverse();
      let trend = null;
      if (latest) for (const prior of priorPool) { trend = comparison(latest, prior, filters); if (trend) break; }
      ...
      const row = { key, student: recent, history, latest, measuredAttempt, measured, level: levelFor(measured.percent), needsSupport: needsSupport(measured.percent), trend, skills, repeated, resolved };
