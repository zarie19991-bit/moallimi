...
function validateIndicators(meta:any,raw:any[],catalog:any[]){
 if(!Array.isArray(raw)||raw.length<1||raw.length>12)throw Object.assign(new Error("اختر من مؤشر واحد إلى اثني عشر مؤشرًا."),{status:400});
 ...
 if(out.length<1||out.length>12)throw Object.assign(new Error("اختر من مؤشر واحد إلى اثني عشر مؤشرًا مختلفًا."),{status:400});
 return out;
}
...
async function buildQuestions(meta:any,inds:any[],seasonId:string){
 ...
 const row=(x:any,stageNo:number,stageType:string,indicatorOrder:number,indicatorQuestionNo:number,isFinal:boolean)=>({
   ...
   explanation:x.explanation||"",difficulty:x.difficulty||null,stage_no:stageNo,stage_type:stageType,indicator_order:indicatorOrder,indicator_question_no:indicatorQuestionNo,is_final:isFinal
 });
 ...
 let bossCount=0;
 const bossTarget=Math.min(3,Math.max(1,inds.length)),bossLevels=["reasoning","application","reasoning"],bossStage=inds.length+1;
 for(let bi=0;bi<bossTarget;bi++){
...
async function indicatorGuide(subject:string,outcome:string,index:number){
 const {data,error}=await db.from("lugati_pretest_templates")
   .select("student_title,golden_rule,common_trap,recognition_cues,solution_steps,question_patterns,transfer_rule,indicator_text")
   .eq("subject_key",subject).eq("outcome_code",outcome).eq("indicator_index",index).eq("is_active",true)
   .order("quality_passed",{ascending:false}).order("updated_at",{ascending:false}).limit(1).maybeSingle();
 if(error)throw error;
 if(!data)return{student_title:"المؤشر الحالي",golden_rule:"حدّد المطلوب، ثم استخدم المعطيات قبل اختيار الإجابة.",common_trap:"لا تتعجل في اختيار أول إجابة تبدو مناسبة.",recognition_cues:[],solution_steps:["حدّد المطلوب.","ارجع إلى المعطيات.","استبعد المشتتات.","تحقق من الإجابة."],question_patterns:[],transfer_rule:"طبّق الفكرة نفسها حتى لو تغيّرت صياغة السؤال."};
 return{
   student_title:data.student_title||data.indicator_text||"المؤشر الحالي",
   golden_rule:data.golden_rule||"",
   common_trap:data.common_trap||"",
   recognition_cues:Array.isArray(data.recognition_cues)?data.recognition_cues:[],
   solution_steps:Array.isArray(data.solution_steps)?data.solution_steps:[],
   question_patterns:Array.isArray(data.question_patterns)?data.question_patterns:[],
   transfer_rule:data.transfer_rule||""
 };
}
function safeRound(r:any){return{id:r.id,arena_key:r.arena_key,subject_key:r.subject_key,title:r.title,status:r.status,effective_status:effectiveStatus(r),open_at:r.open_at,close_at:r.close_at,closed_at:r.closed_at,selected_indicators:r.selected_indicators,question_count:r.question_count,points_per_correct:r.points_per_correct,game_config:r.game_config||{}}}
...
async function createRound(req:Request,b:any,a:Access){
 ...
 const openAt=b?.open_at?new Date(b.open_at):new Date();if(Number.isNaN(openAt.getTime()))throw Object.assign(new Error("وقت فتح اللعبة غير صحيح."),{status:400});
 const closeAt=b?.close_at?new Date(b.close_at):null;if(closeAt&&Number.isNaN(closeAt.getTime()))throw Object.assign(new Error("وقت إغلاق اللعبة غير صحيح."),{status:400});
...
async function roundResults(req:Request,b:any,a:Access){
 ...
 const myResult=me?{...me,max_streak:Number(at?.max_streak||0),points:Number(at?.points||me.points||0)}:null;
 return json(req,{ok:true,round:safeRound(r),me:myResult,total_participants:boards.students.length,top_students:(boards.students||[]).slice(0,10),class_leaderboard:boards.classes,indicator_analysis:analysis});
}
...
async function currentPayload(r:any,at:any){
 if(at.status!=="in_progress")return{finished:true,attempt:{status:at.status,max_streak:at.max_streak,points:at.points}};
 const pos=Number(at.current_position||1);if(pos>Number(r.question_count||15)){at=await finalizeAttempt(at);return{finished:true,attempt:{status:at.status,max_streak:at.max_streak,points:at.points}}}
 ...
 const {data:q,error:qe}=await db.from("lugati_competition_round_questions")
   .select("id,position,indicator_index,indicator_text,outcome_code,cognitive_level,context_text,question_text,options,image,stage_no,stage_type,indicator_order,indicator_question_no,is_final")
   .eq("id",ans.round_question_id).single();if(qe)throw qe;
 ...
 const guide=await indicatorGuide(r.subject_key,q.outcome_code,Number(q.indicator_index));
 const stageNo=Number(q.stage_no||1),stageType=q.stage_type||"indicator",stageTotal=Number(r.game_config?.stage_count||1);
 return{
   finished:false,
   attempt:{id:at.id,current_position:pos,correct_questions:at.correct_questions,wrong_attempts:at.wrong_attempts,started_at:at.started_at,streak:Number(at.streak||0),max_streak:Number(at.max_streak||0),points:Number(at.points||0),powerups:at.powerups||{},active_powerup:at.active_powerup||null},
   stage:{no:stageNo,total:stageTotal,type:stageType,indicator_order:Number(q.indicator_order||1),question_no:Number(q.indicator_question_no||1),is_final:q.is_final===true,title:stageType==="boss"?"المواجهة النهائية":guide.student_title},
   question:{id:q.id,position:pos,indicator_index:q.indicator_index,indicator_text:q.indicator_text,outcome_code:q.outcome_code,cognitive_level:q.cognitive_level,context_text:q.context_text,question_text:q.question_text,image:q.image,options,hearts_remaining:3-Number(ans.wrong_attempts||0),tried_options:ans.selected_history||[],powerup_log:ans.powerup_log||{}},
   guide,elapsed_ms:Math.max(0,Date.now()-new Date(at.started_at).getTime())
 };
}
...
async function usePowerup(req:Request,b:any,a:Access){
 if(a.role!=="student")return json(req,{error:"هذه العملية للطالب فقط."},403);
 const type=tidy(b?.type);if(!["eliminate","hint","double"].includes(type))return json(req,{error:"بطاقة المساعدة غير معروفة."},400);
 const r=await getRound(tidy(b?.round_id));if(r.status!=="open")return json(req,{error:"الجولة غير مفتوحة الآن."},409);
 const {data:at,error}=await db.from("lugati_competition_attempts").select("*").eq("round_id",r.id).eq("student_id",a.student_id).maybeSingle();if(error)throw error;
 if(!at||at.status!=="in_progress")return json(req,{error:"ابدأ الجولة أولًا."},409);
 const {data:ans,error:ae}=await db.from("lugati_competition_attempt_answers").select("*").eq("attempt_id",at.id).eq("position",at.current_position).single();if(ae)throw ae;
 if(ans.completed)return json(req,{error:"انتقل للسؤال التالي لاستخدام البطاقة."},409);
 const log=(ans.powerup_log&&typeof ans.powerup_log==="object")?ans.powerup_log:{};
 if(log[type]!==undefined){
   return json(req,{ok:true,type,already_used:true,result:log[type],powerups:at.powerups||{},active_powerup:at.active_powerup||null});
 }
 const remaining=Number(at.powerups?.[type]||0);if(remaining<1)return json(req,{error:"استخدمت هذه البطاقة في الجولة."},409);
 const {data:q,error:qe}=await db.from("lugati_competition_round_questions").select("*").eq("id",ans.round_question_id).single();if(qe)throw qe;
 const nextPowerups={...(at.powerups||{}),[type]:remaining-1};let result:any=true,newLog:any={...log};
 if(type==="eliminate"){
   const order=(Array.isArray(ans.option_order)?ans.option_order:[0,1,2,3]).map(Number),correctDisplay=order.findIndex((x:number)=>x===Number(q.correct_index));
   const wrong=[0,1,2,3].filter(x=>x!==correctDisplay),pick=wrong[(Number(ans.position||1)+String(q.id).charCodeAt(0))%wrong.length];
   result=[pick];newLog.eliminate=result;
 }else if(type==="hint"){
   const g=await indicatorGuide(r.subject_key,q.outcome_code,Number(q.indicator_index));
   result=(g.recognition_cues?.[0]||g.golden_rule||"حدّد المطلوب ثم ارجع إلى المعطيات المرتبطة به.");newLog.hint=result;
 }else{
   result=true;newLog.double=true;
 }
 const ua=await db.from("lugati_competition_attempt_answers").update({powerup_log:newLog,updated_at:new Date().toISOString()}).eq("id",ans.id);if(ua.error)throw ua.error;
 const up:any={powerups:nextPowerups,updated_at:new Date().toISOString()};if(type==="double")up.active_powerup="double";
 const ut=await db.from("lugati_competition_attempts").update(up).eq("id",at.id);if(ut.error)throw ut.error;
 return json(req,{ok:true,type,result,powerups:nextPowerups,active_powerup:type==="double"?"double":at.active_powerup||null});
}
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(act==="use_powerup")return await usePowerup(req,b,a);
