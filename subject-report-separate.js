...
let attempts=[],tests=[],loaded=false,loading=false,reportHtml='',buildVersion=0;
...
async function build(){
 const version=++buildVersion;reportHtml='';$('printSubjectReportBtn').disabled=true;
 const subject=$('reportSubjectSelect').value,testId=$('reportSubjectTest').value,className=$('reportSubjectClass').value;
 ...
 const test=tests.find(t=>String(t.id)===String(testId))||{},recs=latestRecords(testId,subject,className),s=settings();
 $('subjectReportState').textContent='جارٍ إنشاء التقرير وكشف غير المختبرين…';
 const absenceHtml=await window.NafesReportAbsentees.create({attempts,tests,selectedIds:[testId],className,subject,settings:s});
 if(version!==buildVersion)return;
 if(!recs.length){reportHtml=absenceHtml;$('subjectOfficialReport').innerHTML='<div class="report-preview-empty">لا توجد درجات محفوظة فعلية في هذا الاختبار ضمن الفصل المختار.</div>'+reportHtml;$('printSubjectReportBtn').disabled=!reportHtml;$('subjectReportState').textContent='تم إنشاء كشف المشاركة للفصل المختار.';return}
 ...
 const highest=Math.max(...scores),lowest=Math.min(...scores),avg=sum/n,maxTotal=Math.max(...totals);
 const counts=Object.fromEntries(levels.map(l=>[l.key,vals.filter(p=>level(p)===l.key).length])),maxCount=Math.max(1,...Object.values(counts));
 const term=test.term||test.academic_term||'غير محدد',reportClass=className||'كل الفصول';
 const rows=levels.map(l=>`<tr><td><span class="sar-level-tag ${l.key}">${l.label}</span></td><td>${l.range}</td><td>${ar(counts[l.key])}</td></tr>`).join('');
 reportHtml=`<article class="subject-analysis-sheet official-analysis-sheet" data-grade-source="saved" data-class-name="${E(className)}"><header class="sar-head"><div class="sar-admin"><b>المملكة العربية السعودية</b><span>وزارة التعليم</span><span>الإدارة العامة للتعليم بمنطقة نجران</span><span>${E(s.schoolName||'')}</span></div><div class="sar-ministry">${logo(s.ministryLogo,'وزارة التعليم')}</div><div class="sar-form-no">تقرير مادة</div></header><h1>تقرير نتائج اختبار مادة [${E(names[subject])}]</h1><div class="sar-meta"><div><span>المرحلة الدراسية / الصف:</span><b>الثالث المتوسط · ${E(reportClass)}</b></div><div><span>السنة / الفصل الدراسي:</span><b>${E(term)}</b></div><div><span>درجة القياس (الاختبار):</span><b>${ar(maxTotal)}</b></div></div><div class="sar-analysis-grid"><section class="sar-stats"><h2>الإحصائيات الأساسية</h2><div class="sar-stat-list"><div data-participation-kind="total" data-tested-count="${n}"><span>إجمالي عدد الطلاب</span><b>—</b></div><div data-participation-kind="tested"><span>عدد الطلاب المختبرين</span><b>${ar(n)}</b></div><div><span>أعلى درجة</span><b>${ar(highest)}</b></div><div><span>أقل درجة</span><b>${ar(lowest)}</b></div><div><span>متوسط الدرجات</span><b>${ar(avg)}</b></div><div><span>نسبة التحصيل</span><b>${pct(achievement)}</b></div><div><span>مجموع الدرجات</span><b>${ar(sum)}</b></div></div></section><section class="sar-achievement"><h2>الإحصائيات التحصيلية</h2><table><thead><tr><th>المستوى</th><th>النطاق</th><th>عدد الطلاب</th></tr></thead><tbody>${rows}</tbody></table></section></div><section class="sar-chart-card"><h2>رسم بياني (نسب الطلاب لكل تقدير)</h2><div class="sar-rings">${levels.map(l=>ring(l.label,counts[l.key],n,l.key)).join('')}</div></section><section class="sar-chart-card"><h2>رسم بياني (عدد الطلاب لكل تقدير)</h2><div class="sar-bars">${levels.map(l=>bar(l.label,counts[l.key],maxCount,l.key)).join('')}</div></section><footer class="sar-signatures"><div><b>معلم/ة المادة:</b><span>${E(s.teacherName||'________________')}</span></div><div><b>مدير/ة المدرسة:</b><span>${E(s.principalName||'________________')}</span></div></footer><section class="sar-student-table"><h2>تفاصيل الدرجات المحفوظة للطلاب في ${E(names[subject])}</h2><table><thead><tr><th>م</th><th>اسم الطالب</th><th>الفصل</th><th>الدرجة</th><th>النسبة</th><th>التقدير</th></tr></thead><tbody>${recs.sort((a,b)=>b.m.percent-a.m.percent).map((x,i)=>`<tr><td>${ar(i+1)}</td><td>${E(studentName(x.a))}</td><td>${E(x.a.class_name||'—')}</td><td>${ar(x.m.score)} / ${ar(x.m.total)}</td><td>${pct(x.m.percent)}</td><td>${levels.find(l=>l.key===level(x.m.percent)).label}</td></tr>`).join('')}</tbody></table></section></article>`;
 reportHtml+=absenceHtml;$('subjectOfficialReport').innerHTML=reportHtml;$('printSubjectReportBtn').disabled=false;$('subjectReportState').textContent=`تم إنشاء تقرير ${names[subject]} من الدرجات المحفوظة لعدد ${ar(n)} طالبًا، ونسبة التحصيل ${pct(achievement)}.`;
}
async function load(){if(loading||!T?.getKey?.())return;loading=true;try{let cursor=0,all=[],ts=[];do{const d=await T.api('teacher_data',{cursor,limit:100});all.push(...(d.attempts||[]));if(cursor===0)ts=d.tests||[];cursor=d.next_cursor}while(cursor!==null);attempts=all.map(x=>A?.normalizeAttempt?A.normalizeAttempt(x):x);tests=ts;loaded=true;fillTests()}catch(e){$('subjectReportState').textContent='تعذر تحميل بيانات تقرير المادة: '+(e.message||e)}finally{loading=false}}
function init(){if(!$('reportSubjectTest'))return;$('reportSubjectSelect').addEventListener('change',()=>{if(loaded)fillTests();else load()});$('reportSubjectTest').addEventListener('change',invalidate);$('reportSubjectClass').addEventListener('change',()=>{if($('reportSubjectTest').value)build();else invalidate()});$('buildSubjectReportBtn').onclick=build;$('printSubjectReportBtn').onclick=()=>{if(!reportHtml)return;const root=$('printRoot');root.innerHTML=reportHtml;root.setAttribute('aria-hidden','false');window.print()};load()}
addEventListener('nafes:auth-changed',e=>{if(e.detail?.authenticated){loaded=false;load()}});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
})();
